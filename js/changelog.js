...
window.CHANGELOG = [
  {
    v: '3.8',
    data: 'agosto 2026',
    titolo: 'La barca di Elio torna in acqua, e la valle si disegna in un altro modo',
    voci: [
      { tipo:'nuovo',  t:'La barca di Elio torna in acqua, e porta in due posti nuovi', d:'Dopo la veglia al Santuario il paese si chiede da dove ripartire, e alla bacheca della piazza compare il primo progetto: rimettere in acqua la barca del padre di Elio. Non per andare via — <b>le rotte sono brevi e servono a qualcosa</b>, e c\'è sempre una strada per tornare. La vicenda passa da Tobia per il materiale e da Serafina per la carta delle cale, poi chiede di stare alla Costa <b>con la bassa marea, di mattina</b>, e di leggere i tre segni nell\'ordine giusto: la conchiglia, la pietra chiara, il segno della rete. Sbagliare non punisce: si ricomincia dalla conchiglia. Da lì la barca porta alla <b>Cala delle Reti</b>, e più avanti all\'<b>Approdo delle Erbe</b> — due posti piccoli di proposito, la pineta alle spalle e la riva che sale e scende invece di correre dritta.' },
      { tipo:'nuovo',  t:'La posta attraversa, e l\'approdo si apre', d:'La bacheca dei progetti non si ferma alla barca. Quando la Cala è raggiungibile, Marisol propone di portarci una lettera, e la cassa di legno accanto al molo diventa una <b>cassetta della posta</b> vera: da lì in poi le notizie fanno una strada corta fra le due rive. Dopo quella, Elio propone di rinforzare un ormeggio oltre la Cala, e nasce l\'Approdo delle Erbe. Ogni progetto si segna dalla bacheca e poi si porta avanti parlando con chi l\'ha proposto; quelli non ancora aperti restano in elenco con scritto cosa manca, invece di non esserci — sapere che una cosa arriverà è metà del motivo per arrivarci.' },
      { tipo:'meglio', t:'La valle si compone in WebGL', d:'Il gioco disegnava tutto su una tela sola, un passaggio dopo l\'altro. Andava bene finché la composizione era semplice, e ha smesso di andare bene quando sopra al mondo si sono messe in fila le ombre che seguono il sole, il bloom, la gradazione, i raggi di luce e la vignetta: sono passaggi <b>a schermo pieno</b>, e su una tela 2D si pagano tutti a ogni fotogramma. Adesso la scena la mette insieme <b>PixiJS 8 in WebGL</b>, su tredici livelli separati. L\'arte non è stata riscritta: le stesse funzioni di sempre riempiono una tela fuori schermo, e quella diventa una texture — è cambiato solo chi impila i pezzi alla fine. <b>Se WebGL non c\'è, il gioco torna alla tela di prima</b> e si gioca uguale; che le due strade disegnino la stessa identica cosa non è una speranza, è verificato confrontando i pixel di dodici scene.' },
      { tipo:'nuovo',  t:'Il personaggio è disegnato a mano', d:'Era generato in codice come tutto il resto, e su una casella da 64 pixel si vedeva. Adesso ha otto fogli disegnati: la camminata a mani vuote e uno per ogni attrezzo che si tiene in mano — zappa, ascia, piccone, falce, canna, arco, annaffiatoio — con le pose per tutte le direzioni. La falcata, l\'appoggio a terra e la trasparenza di ogni foglio si controllano a ogni pubblicazione: una posa storta in un foglio solo non è il genere di cosa che si nota leggendo il codice.' },
      { tipo:'nuovo',  t:'La staccionata è disegnata, e sa in che fila si trova', d:'Le tratte dritte, gli angoli, le T e la croce arrivano da una tavola disegnata a mano: nove configurazioni di lati su sedici, con l\'angolo di nord-ovest ricavato ribaltando quello di nord-est. Le combinazioni che restano — i tratti che finiscono nel vuoto, e il cancelletto — continuano a essere disegnate in codice come prima. Quindi una fila mista non ha buchi: cambia il tratto, non la staccionata.' },
      { tipo:'nuovo',  t:'A livello 3 il mestiere prende una direzione', d:'Salire di livello dava numeri e basta. Adesso al terzo livello di <b>Agricoltura, Raccolta, Estrazione e Pesca</b> si sceglie una specializzazione fra due — il Coltivatore contro il Custode della terra, il Botanico contro l\'Esploratore, il Fabbro di vena contro il Cercatore, il Pescatore contro il Navigatore — e la scelta si rilegge sempre dal Diario, quindi chiudere la carta subito non costa niente. <b>E non è per sempre</b>: quando le quattro braci del Santuario sono accese, la Lanterna concede <b>una sola</b> riconsiderazione, che cambia direzione senza toccare né i livelli né l\'esperienza.' },
      { tipo:'meglio', t:'I cinque pulsanti dell\'HUD sono illustrati', d:'Zaino, artigianato, diario, mappa e menu erano cinque emoji, e un\'emoji la disegna il sistema operativo: su ogni telefono uscivano di uno stile diverso da tutto il resto del gioco. Adesso sono cinque pulsanti disegnati, a 128 pixel perché si mostrano a misure diverse e su uno schermo a densità doppia quelle misure raddoppiano. Se un file tarda o manca, sotto c\'è ancora l\'emoji di prima: è più utile di un quadrato vuoto.' },
      { tipo:'fix',    t:'700 KB scaricati per niente, a ogni partita', d:'Il primo tentativo di staccionata disegnata era un\'altra tavola, superata da quella che si usa adesso. La tavola vecchia era rimasta nell\'elenco delle cose da caricare all\'avvio: ogni partita se la scaricava e la teneva in memoria — misurato, arrivava davvero, 700 KB — e non la disegnava nessuno. È il difetto che non si vede, perché a schermo non manca niente: si nota solo come un\'attesa in più all\'apertura, e sul telefono come traffico.' }
    ]
  },
  {
    v: '3.7',
    ...
    voci: [
      { tipo:'nuovo',  t:'Il pacco della valle: sette giorni, sette pacchi', d:'Chi torna al podere trova un pacco sull\'aia, uno per <b>giorno vero</b> — se apri Fioralba oggi e poi domani, sono due. Si apre cliccandoci sopra, e quello che c\'è dentro esce una carta per volta: il settimo giorno il pacco è grosso, ha il fiocco e dà 2000 monete, due lingotti d\'oro, una gemma di luna e una torta di zucca. Poi il giro ricomincia. I sette bollini in cima dicono a che punto sei senza doverlo scrivere. <b>Saltare un giorno costa un passo, non tutto</b>: si torna indietro di un giorno per ogni giorno saltato, col fondo a uno — una dimenticanza costa poco, e non c\'è nessuna settimana da perdere. Dentro ci sono cose che il gioco già dà, mai roba che si trova solo così: collegarsi ogni giorno è un di più, non la via principale per niente.' },
